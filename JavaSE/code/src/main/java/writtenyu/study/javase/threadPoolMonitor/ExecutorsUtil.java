package writtenyu.study.javase.threadPoolMonitor;
...
public class ExecutorsUtil extends ThreadPoolExecutor {
     ...
     */
    public ExecutorsUtil(int corePoolSize, int maximumPoolSize, long keepAliveTime, TimeUnit unit, BlockingQueue
            workQueue, String poolName) {
        super(corePoolSize, maximumPoolSize, keepAliveTime, unit, workQueue, new EventThreadFactory(poolName));
        this.startTimes = new ConcurrentHashMap<>();
        this.poolName = poolName;
        /**
         这里看 ThreadPoolExecutor 最全的一个构造器
         目的是熟知构造器的核心参数有哪些？各自的作用是什么？
        int corePoolSize, 核心 线程池核心线程数
            这个核心线程通过 allowCoreThreadTimeOut() 和  keepAliveTime() 方法 来 控制核心线程数 是否可以销毁 和 空闲超时退出时间
            corePoolSize 并不会在 线程池创建初始 就创建出来 除非 调用了prestartCoreThread/prestartAllCoreThreads事先启动核心线程
        int maximumPoolSize,
            如果队列中任务已满，注意是队列任务已满才行，并且当前线程个数小于maximumPoolSize，那么会创建新的线程来执行任务
        long keepAliveTime,
        TimeUnit unit,
    ...
    public void shutdown() {
        ...
        LOGGER.info(String.format(this.poolName + " Going to shutdown. Executed tasks: %d, Running tasks: %d, Pending" +
            " tasks: %d", this.getCompletedTaskCount(), this.getActiveCount(), this.getQueue().size()));
        super.shutdown();
    }
    ...
    public List shutdownNow() {
        // 统计已执行任务、正在执行任务、未执行任务数量
        LOGGER.info(String.format(this.poolName + " Going to immediately shutdown. Executed tasks: %d, Running tasks: %d," +
            " Pending tasks: %d", this.getCompletedTaskCount(), this.getActiveCount(), this.getQueue().size()));
    ...
    protected void afterExecute(Runnable r, Throwable t) {
        ...
        LOGGER.info(String.format(this.poolName
            ...
            " Task: %d, Queue: %d, LargestPoolSize: %d, MaximumPoolSize: %d,KeepAliveTime: %d, " +
            "isShutdown: %s, isTerminated: %s",
            diff, this.getPoolSize(), this.getCorePoolSize(), this.getActiveCount(), this.getCompletedTaskCount(),
