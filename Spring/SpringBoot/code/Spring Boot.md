...
- @SpringBootApplication ： 标注springboot的主程序类
...
- @SpringBootConfiguration : 实际上就是标注一个配置类
...
@Documented
@Inherited
@AutoConfigurationPackage 启用主配置类所在包及子包为扫描包
...
- @Import(AutoConfigurationImportSelector.class)  ---- 这个比较重要

springboot自动配置类的加载

springboot 从 ```org.springframework.core.io.support.SpringFactoriesLoader#loadSpringFactories``` 这个方法开始从类路径下的 ```META-INF/spring.factories```加载自动配置类
...
- @PropertySource&@ImportResource&@Bean
  ...
  - @PropertySource : 指定除了主配置文件以外的额外配置文件

  - @ImportResource ： 导入额外的spring配置文件（*.xml）
...
​	项目打包好以后，我们可以使用命令行参数的形式，启动项目的时候

来指定配置文件的新位置；指定配置文件和默 认加载的这些配置文件共同
...
11. 通过SpringApplication.setDefaultProperties指定的默认属性 所有支持的配置加载来源    
    ...
    [参考官方文档](https://docs.spring.io/spring-boot/docs/2.0.3.RELEASE/reference/htmlsingle/#boot-features-external-config)

## 6. 自动配置原理    

​	配置文件到底能写什么？怎么写？自动配置原理；

​	[配置文件能配置的属性参照](https://docs.spring.io/spring-boot/docs/2.0.3.RELEASE/reference/htmlsingle/#common-application-properties)

### 6.1 自动配置原理

1). springboot 在加载主配置类的时候，使用@EnableAutoConfiguration开启了自动配置功能

2). @EnableAutoConfiguration 使用 AutoConfigurationImportSelector 给容器中导入一系列组件
...
​            getCandidateConfigurations 方法获取候选组件
...
org.springframework.boot.autoconfigure.dao.PersistenceExceptionTranslationAutoConfigurati
.......
```
...
​	5). 以**HttpEncodingAutoConfiguration（Http编码自动配置）**为例解释自动配置原理；    

```java
//表示这是一个配置类，以前编写的配置文件一样，也可以给容器中添加组件
@Configuration
// 启动指定类的 ConfigurationProperties功能；将配置文件中对应的值和HttpEncodingProperties绑定起来；并把 HttpEncodingProperties 加入到ioc容器中
...
​	6). 所有在配置文件中能配置的属性都是在xxxxProperties类中封装者‘；配置文件能配置什么就可以参照某个功 能对应的这个属性类    
...
![slf4j替换其他日志](C:\Users\wr1ttenyu\学习\study-record\Spring\SpringBoot\img\slf4j替换其他日志.png)   
...
![springboot日志底层依赖关系](..\img\springboot日志底层依赖关系.png)
...
​		分为两种：一种是日志实现非slf4j系，但日志门面为slf4j，使用中间包来做适配；另一种是，应用日志实现是slf4j系，但应用依赖的其他工程的日志门面为非slf4j，使用中间包来做适配

​		具体可参考：**“2、SLF4j使用”** 小节中的==两张图==
...
JSP、Velocity、Freemarker、Thymeleaf    ![模板引擎](..\img\模板引擎.png)
...
​	  th：任意html属性；来替换原生属性的值    ![thymeleaf语法](..\img\thymeleaf语法.png)

2）、表达式    

[参考官方pdf](https://www.thymeleaf.org/doc/tutorials/3.0/usingthymeleaf.pdf) (跟我说你不会英文？干劲学去，傻逼东西！)
...
![默认html错误页面](..\img\默认html错误页面.png)

​	   浏览器发送请求的请求头：    

![浏览器请求头](..\img\浏览器请求头.png)

2）、如果是其他客户端，默认响应一个json数据    

![默认json错误返回](..\img\默认json错误返回.png)

​	  其他工具请求头

![其他工具请求头](..\img\其他工具请求头.png)

原理： 

可以参照ErrorMvcAutoConfiguration；

错误处理的自动配置； 给容器中添加了以下组件
...
- 我们可以使用4xx和5xx作为错误页面的文件名来匹配这种类型的所有错误，精确优先（优先寻找精确的状态 码.html）； 页面能获取的信息； 
  ...
  //没有自适应效果...
  //自适应效果是指  根据请求头  判断返回是HTML 还是json
  ```
...
![springboot默认支持的servlet容器](..\img\springboot默认支持的servlet容器.png)
...
// 导入springboot支持的三种servlet容器的配置类
...
以ServletWebServerFactoryConfiguration.EmbeddedTomcat为例：
...
2). 在WebServerFactoryCustomizerBeanPostProcessor调用定制器的customize()方法

​	我们可以通过自己定制WebServerFactoryCustomizer 或者直接使用 ServerProperties 来修改容器配置
...
​	通过 WebServerFactoryCustomizerBeanPostProcessor，在bean的创建过程中，一但发现是WebServerFactory就获取容器中所有的WebServerFactoryCustomizer，对WebServerFactory进行属性赋值
...
2）、refreshContext(context);SpringBoot刷新IOC容器【创建IOC容器对象，并初始化容器，创建容器中的每一 个组件】；如果是web应用创建AnnotationConfigServletWebServerApplicationContext，否则： AnnotationConfigApplicationContext

3）、refresh(context);刷新刚才创建好的ioc容器；    

- 判断项目中是否含有 **javax.servlet.Servlet 或者 org.springframework.web.context.ConfigurableWebApplicationContext**
...
- 如果是web项目则创建 **AnnotationConfigServletWebServerApplicationContext**
...
- 在createWebServer()方法中获取web容器时，触发**org.springframework.boot.web.server.WebServerFactoryCustomizerBeanPostProcessor**，来加载容器配置，并从工厂中获取server实例，启动server容器
  ...
  private void createWebServer() {
      ...
      if (webServer == null && servletContext == null) {
          // 从spring容器中获取server工厂，bean创建触发后置处理器，加载容器配置
          ServletWebServerFactory factory = getWebServerFactory();
          // 从工厂中获取server实例，并启动server容器
...
- 流程： 
  ...
  4）、每一个WebApplicationInitializer都调用自己的onStartup；    ![WebApplicationInitializer实现类](..\img\WebApplicationInitializer实现类.png)
  ...
  protected WebApplicationContext createRootApplicationContext(
      ...
      // 创建createSpringApplicationBuilder
...
![docker的icon](..\img\docker的icon.png)

![docker运行原理图](..\img\docker运行原理图.png)
...
![docker运行示意图](..\img\docker运行示意图.png)
...
![springdata-JPA](..\img\springdata-JPA.png)
...
public ConfigurableApplicationContext run(String... args) {
	...
	try {
		...
		ConfigurableEnvironment environment = prepareEnvironment(listeners,
				applicationArguments);
		// TODO(what is the meaning of this value) spring.beaninfo.ignore 值塞入System中
		configureIgnoreBeanInfo(environment);
		Banner printedBanner = printBanner(environment);
...
public class MySpringApplicationRunListener implements SpringApplicationRunListener {
    ...
    public MySpringApplicationRunListener(SpringApplication springApplication, 
		String[] args) {}
    ...
    public void environmentPrepared(ConfigurableEnvironment environment) {
        System.out.println("MySpringApplicationRunListener... environment 
			prepared...");
    }
...
wr1ttenyu.study.springboot.demo.curd.listener.MySpringApplicationContextInitializer

# Application Listeners
org.springframework.boot.SpringApplicationRunListener=\
wr1ttenyu.study.springboot.demo.curd.listener.MySpringApplicationRunListener
```
...
public class MySpringCommandLineRunner implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        System.out.println("MySpringCommandLineRunner... run ..." + 
			Arrays.asList(args));
    }
}
```
